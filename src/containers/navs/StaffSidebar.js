...
import menuItems from '../../constants/staffMenu';
                ...
                {menuItems &&
                  menuItems.map((item) => {
                    if(item.ref==='Dashboard') {
                      ...
                      const { permission } = this.state;
                      if (permission[item.pindex].status==="2" || permission[item.pindex].status==="3") {
                        return (
                          <NavItem
                            key={item.id}
                            className={classnames({
                    ...
                    >
                      {item.subs &&
                        item.subs.map((sub, index) => {
                          const { permission } = this.state;
                          if (permission[item.pindex].status==="2") {
                            if(sub.type === "all" || sub.type === "read") {
                              return (
                                <NavItem
                                  key={`${item.id}_${index}`}
                                  className={`${
                          ...
                          } else if (permission[item.pindex].status==="3") {
                            return (
                              <NavItem
                                key={`${item.id}_${index}`}
                                className={`${

...
const Table = ({ columns, data, divided = false, defaultPageSize, deleteRestaurant, gotoPage ,pageIndex, pageCount, canNextPage,canPreviousPage, onSort }) => {
  const {
    getTableProps,
    getTableBodyProps,
    prepareRow,
          ...
          {page.map((row) => {
            ...
            return (
              <tr {...row.getRowProps()}>
                ...
                <td>
                   <NavLink to={`/vendor/restaurant/dashboard/${row.original.id}`} className="btn btn-success">Enter</NavLink>{" "}

                   <NavLink to={`/vendor/restaurants/update/${row.original.id}`} className="btn btn-primary">Edit</NavLink>{" "}
                   
                   <Button color="danger" onClick={()=> deleteRestaurant(row.original.id)}>Delete</Button>
                </td>
              </tr>
            );
          })}
...
class VendorList extends React.Component {
  ...
  bindData() {
    Api.restaurantList(this.state.filter)
  ...
  render() {
    ...
    const cols = [
        ...
        },
        {
          ...
          cellClass: 'list-item-heading w-30',
          sortClass: filter.sortParameter.field==='phone_number'? filter.sortParameter.type : '' //*to set the sort select class *//
        }
      ];
            ...
            <Card className="mb-4">
              <CardBody>
                <Row>
                  <Colxx xxs="10">
                    <CardTitle>
                      <i className="iconsminds-align-justify-all"></i>&nbsp;&nbsp;Restaurant List
                    </CardTitle>
                  </Colxx>
                </Row>
                ...
                <Table columns={cols} data={data} defaultPageSize={filter.sizePerPage} gotoPage={this.gotoPage}
                  ...
                  canNextPage={canNextPage} onSort={this.onSort} deleteRestaurant={this.deleteRestaurant} />
              </CardBody>
            </Card>
          </Colxx>
        </Row>

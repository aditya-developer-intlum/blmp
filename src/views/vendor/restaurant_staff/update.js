  ...
  bindData() {
    ...
    .then(async response => {
      if (response.status === 200) {
        ...
        ApiRestaurantStaff.show(this.props.match.params.id)
        .then(async response => {
        if (response.status === 200) {
          ...
          this.setState({
            form: {
              ...
              associate_restaurant: JSON.parse(data.associate_restaurant),
              status: data.status,
              permissions: JSON.parse(data['permissions']['permissions']),
              notifications: JSON.parse(data['permissions']['notifications'])
            }
          })
        } else {
          this.props.history.push('/vendor/restaurant_staff/list/')
          NotificationManager.error('', 'No content found.', 3000, null, null, '');
        }
      })
    })
  }
  onRegister = async (values) => {
    if (!this.props.loading) {
      ...
      if (error === 0) {
        ...
        //console.log(values);
          ApiRestaurantStaff.update(this.props.match.params.id,values)
          .then(async response => response.data)
          .then(async data => {
            if (data) {
              await NotificationManager.success('', 'Restaurant Staff Updation Successful!', 3000, null, null, '');
              this.props.history.push('/vendor/restaurant_staff/list')
            } else {
              NotificationManager.error(Array.isArray(data.error) ? data.error.map(val => val.message).join(" , ") : '', 'Restaurant Staff Updation Failed! Please try again.', 3000, null, null, '');
            }
          })
          .catch(async error => {
            NotificationManager.error('', 'Restaurant Staff Updation Failed! Please try again.', 3000, null, null, '');
          });
      } else {
        this.setState({
          errors: errors,
            ...
            <Card className="mb-4">
              <CardBody>
                <Row className="mb-2">
                  <Colxx xxs="10">
                    <CardTitle>
                      <i className="iconsminds-add"></i>&nbsp;&nbsp;Update Restaurant Staff
                    </CardTitle>
                  </Colxx>
                </Row>

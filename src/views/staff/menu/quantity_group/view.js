    ...
    Api.getDataById(this.props.match.params.item)
      .then(async response => {
        ...
        if (response.status === 200) {
          ...
          this.props.history.push('/staff/menu/quantity_group/list/')
            ...
            <Card className="mb-4">
              <CardBody>
                <Row className="mb-2">
                  <Colxx xxs="10">
                    <CardTitle>
                      <i className="iconsminds-add"></i>&nbsp;&nbsp;View Quantity Group
                    </CardTitle>
                  </Colxx>
                </Row>
                <QuantityGroupForm loading={this.props.loading} mode={'view'} formFields={form}
